...
const SummaryPanel: React.FC<{
    ...
    showSpot: 'day_ahead' | 'real_time' | null;
}> = ({ selectedCurves, showSpot }) => {
    const spotLabel = showSpot === 'day_ahead' ? '日前现货' : '实时现货';
...
const FilterPanel: React.FC<{
    ...
    showSpot: 'day_ahead' | 'real_time' | null;
    onShowSpotChange: (show: 'day_ahead' | 'real_time' | null) => void;
    availableCurves: string[];
}> = ({ selectedCurves, onSelectedCurvesChange, showSpot, onShowSpotChange, availableCurves }) => {
        ...
        <Paper variant="outlined" sx={{ p: 2, mt: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, pt: 1.5, mt: 1.5, borderTop: '1px dashed', borderColor: 'divider' }}>
                ...
                <Chip
                    label="日前现货"
                    size="small"
                    onClick={() => onShowSpotChange(showSpot === 'day_ahead' ? null : 'day_ahead')}
                    sx={{
                        backgroundColor: showSpot === 'day_ahead' ? '#f44336' : 'transparent',
                        color: showSpot === 'day_ahead' ? 'white' : 'text.primary',
                        border: `1px solid ${showSpot === 'day_ahead' ? '#f44336' : '#ccc'}`,
                        fontWeight: showSpot === 'day_ahead' ? 'bold' : 'normal',
                        cursor: 'pointer',
                        '&:hover': {
                            backgroundColor: showSpot === 'day_ahead' ? '#f44336' : 'action.hover',
                            opacity: showSpot === 'day_ahead' ? 0.9 : 1
                        }
                    }}
                />
                <Chip
                    label="实时现货"
                    size="small"
                    onClick={() => onShowSpotChange(showSpot === 'real_time' ? null : 'real_time')}
                    sx={{
                        backgroundColor: showSpot === 'real_time' ? '#e91e63' : 'transparent',
                        color: showSpot === 'real_time' ? 'white' : 'text.primary',
                        border: `1px solid ${showSpot === 'real_time' ? '#e91e63' : '#ccc'}`,
                        fontWeight: showSpot === 'real_time' ? 'bold' : 'normal',
                        cursor: 'pointer',
                        '&:hover': {
                            backgroundColor: showSpot === 'real_time' ? '#e91e63' : 'action.hover',
                            opacity: showSpot === 'real_time' ? 0.9 : 1
                        }
                    }}
                />
            </Box>
...
const MultiCurveChart: React.FC<{
    ...
    showSpot: 'day_ahead' | 'real_time' | null;
    dateStr: string;
}> = ({ curvesByPeriod, spotCurves, selectedCurves, showSpot, dateStr }) => {
    ...
    const allPrices = chartData.flatMap(d => {
        ...
        if (showSpot && d['现货'] !== null && d['现货'] !== undefined) {
            prices.push(d['现货']);
        }
        return prices;
    });
...
export const CurveCompareTab: React.FC<CurveCompareTabProps> = ({
    ...
    const [showSpot, setShowSpot] = useState<'day_ahead' | 'real_time' | null>('day_ahead');
