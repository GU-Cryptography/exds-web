...
@router.get("/tou-rules/day", summary="获取指定日期的分时时段定义(96点)")
def get_tou_rule_for_day(date: str = Query(..., description="日期, 格式 YYYY-MM-DD")):
    """
    获取指定日期的分时时段定义，返回 96 点映射列表
    """
    try:
        dt = datetime.strptime(date, "%Y-%m-%d")
        rule_map = get_tou_rule_by_date(dt)
        # 按时间排序并转换为列表
        return [{"time": k, "period_type": v} for k, v in sorted(rule_map.items())]
    except ValueError:
        raise HTTPException(status_code=400, detail="日期格式无效，请使用 YYYY-MM-DD 格式")
    except Exception as e:
        logger.error(f"Error in get_tou_rule_for_day: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"获取分时电电价规则失败: {str(e)}")
