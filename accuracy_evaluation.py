...
from stef.utils.mongo import DATABASE
try:
    from stef.utils.tou_service import get_period_indices_by_date
except ImportError:
    get_period_indices_by_date = None
...
def evaluate_forecast_accuracy(
    ...
    for forecast_id in forecast_ids:
        ...
        doc = {
            ...
            'forecast_date': target_date - timedelta(days=1), # Assuming D-1 forecast
...
def calculate_metrics(y_actual: np.ndarray, y_pred: np.ndarray, target_date: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    if len(y_actual) > 1:
        ...
        # Handle zero diff correctly? Assuming sign(0) is 0. 
        # Match matches if both up, both down, or both flat.
    ...
    # Period Accuracy (TOU)
    period_accuracy = {}
    if target_date and get_period_indices_by_date:
        try:
